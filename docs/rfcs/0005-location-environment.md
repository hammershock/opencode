...
- `/env list`：显示开关、generation、来源、变量名和覆盖关系，默认不显示值；不进入 Session 或模型上下文。用户二次确认后可以在当前临时 dialog 中 reveal 值，关闭 dialog 立即清除展示状态，值不得写入剪贴板、history 或普通日志。
...
- reveal 必须由每次 dialog 的显式二次确认授权，不能成为持久偏好。
- 环境值不进入普通日志或错误对象。
