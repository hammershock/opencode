...
打开 Sync settings 或成功创建、进入、切换 active space 后，如果本机存在未归属 Session，workflow 对当前具体 Session ID 快照显示一次批量提示：`Yes, add all` 或 `No`。关闭提示严格等价 `No`；拒绝后相同 ID 集合再次打开不重复提示，集合变化后可以再次提示。手动 `Sync now` 是明确的重新询问入口，每次执行时只要仍有未归属 Session 就再次提示；启动、后台同步和定时调度不得弹出。overview 也保留显式的 `Add unassigned Sessions...` action。确认 `Yes` 时客户端提交提示时保存的同一组 ID，Core 只处理提交时仍未归属者；期间新建、删除或已改变归属的 Session 不被意外纳入。归属操作必须通过 Session domain workflow 产生初始 outbox，不能由 TUI 直接修改字段。
...
15. 未归属提示提交具体 Session ID 快照；关闭等价 `No`，相同集合拒绝后打开设置不重复、集合变化可重试，手动 Sync Now 每次有未归属 Session 都重新询问，后台调度从不弹窗。
