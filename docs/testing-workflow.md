...
Every functional change in this fork must pass automated verification and real-device acceptance. Unit tests alone are insufficient because Location, SSH/Rexd, TUI input, secure storage, provider authentication and multi-device synchronization depend on real operating-system behavior.
...
- Use a unique temporary test workspace, target label, Session title prefix and sync namespace for each run.
...
- target ID, SSH configuration, credentials, `.env` values and UI/runtime state never upload;
...
This is a release-blocking regression. Payload garbage collection may occur after acknowledgements, but the encrypted deletion marker must remain. Any resurrection is a correctness and data-loss-class failure; do not ship or merge around it.
...
In particular, preserve regression coverage for the legacy sync resurrection failures, TUI-owned remote execution, persisted Shell cwd, control-device `.env` resolution and unencrypted sync metadata. A new implementation passes only when those old failure modes are structurally impossible or covered by a failing test.
