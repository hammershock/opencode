...
describe("SyncSecureStore", () => {
  ...
  test("reads legacy Baidu app credentials from the secure store", async () => {
    ...
    expect(await SyncSecureStore.readLegacyBaiduApp(secure)).toEqual({ appKey: "app", secretKey: "secret" })
